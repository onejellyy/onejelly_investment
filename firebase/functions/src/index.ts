...
const OPENDART_API_KEY = defineSecret("OPENDART_API_KEY");
const CLOUDFLARE_API_URL = defineSecret("CLOUDFLARE_API_URL");
const CLOUDFLARE_API_SECRET = defineSecret("CLOUDFLARE_API_SECRET");
...
type DisclosureCategory = "실적" | "수주계약" | "자본" | "주주가치" | "지배구조" | "리스크" | "기타";
...
class OpenDartClient {
  ...
  async getDisclosureList(params: {
    ...
    return response.json() as Promise<DartListResponse>;
  }
...
// 공시 분류 및 변환
// ============================================
function classifyCategory(title: string): DisclosureCategory {
...
function isKrxCorp(corpCls: string): boolean {
  return corpCls === "Y" || corpCls === "K" || corpCls === "N";
}

function convertToDisclosure(item: DartListItem): Disclosure {
...
// 스케줄러: 5분마다 공시 수집
// ============================================
export const collectDisclosures = onSchedule(
  {
...
// HTTP 트리거: 수동 실행용
// ============================================
export const collectDisclosuresManual = onRequest(
  {
    secrets: [OPENDART_API_KEY, CLOUDFLARE_API_URL, CLOUDFLARE_API_SECRET],
    memory: "256MiB",
    timeoutSeconds: 120,
